...
public class Xliff20LoadSaveTests
{
    ...
    [Test]
    public void LoadAndSave_Xliff20_ShouldBeEquivalent()
    {
        // Arrange
        ...
        var originalUnits = originalXml.Descendants(originalXml.Root!.Name.Namespace + "unit").ToList();
        var savedUnits = savedXml.Descendants(savedXml.Root!.Name.Namespace + "unit").ToList();
        savedUnits.Count.Should().Be(originalUnits.Count, "Should have same number of units");
        ...
        foreach (var originalUnit in originalUnits)
        {
            ...
            var originalSource = originalUnit.Descendants(originalXml.Root!.Name.Namespace + "source").FirstOrDefault()
                ?.Value;
            var savedSource = savedUnit!.Descendants(savedXml.Root!.Name.Namespace + "source").FirstOrDefault()?.Value;
